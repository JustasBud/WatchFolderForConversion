...
# cd '/media/justas/ubuntustorage/TV/fleabag/season 1'
while true
do
    touch  ./lastwatch
    sleep 10
    ...
    find "/volume1/SharedMedia/movies" "/volume1/SharedMedia/tv" -not -path '*/[@.]*' -cnewer ./lastwatch -type f -print0 | while IFS= read -r -d '' file_path; 
    do       
        # echo "$file_path"
        ...
        if [[ "$file_path" == *'.mkv' && ( "$file_path" == *'DV'* || "$file_path" == *'DolbyVision'* || "$file_path" == *'Dolby Vision'* || "$file_path" == *'DoVi'* ) ]]; then
            ...
            echo 'Running dvmkv2mp4.sh for ' "$file_path"
            /volume1/docker/bash_watchfolder_for_dvmkv2mp4_subextract/dvmkv2mp4.sh "$file_path" "$file_dir" &
            ...
            /volume1/docker/bash_watchfolder_for_dvmkv2mp4_subextract/extract_subs.sh "$file_path" "$file_dir"
